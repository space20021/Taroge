    ...
    while(true)
    {
        ...
        for(int j=0; j<12; j++) // read channels data for biniary file , ch 0-11 (12 channels);
        {
            ...
            if (max_143==0&&jiz>100)
            {
                ...
                h2->Smooth(5);
                //Background
                Float_t left[500], left_background[500]; //Cut off 150~164
                Float_t right[500], right_background[500];
                for (int k=0;k<150;k++)
                {
                    left[k]=h2->GetBinContent(k);
                    left_background[k]=h2->GetBinContent(k);
                }
                for (int k=0;k<335;k++)
                {
                    right[k]=h2->GetBinContent(k+165);
                    right_background[k]=h2->GetBinContent(k+165);
                }
                TSpectrum* s1 = new TSpectrum();
                s1->Background(left_background,150,5,TSpectrum::kBackDecreasingWindow,TSpectrum::kBackOrder2,kFALSE,TSpectrum::kBackSmoothing3,kFALSE);
                TSpectrum* s2 = new TSpectrum();
                s2->Background(right_background,335,5,TSpectrum::kBackDecreasingWindow,TSpectrum::kBackOrder2,kFALSE,TSpectrum::kBackSmoothing3,kFALSE);
                TH1F* h3 = new TH1F("h3","background",500,0,500);
                for (int k=0;k<150;k++)
                    h3->SetBinContent(k,left_background[k]);
                for (int k=150;k<165;k++)
                    h3->SetBinContent(k,-100);
                for (int k=165;k<500;k++)
                    h3->SetBinContent(k,right_background[k-165]);
                TH1F* h4 = new TH1F("h4","smooth+remove_background",500,0,500);
                for (int k=0;k<150;k++)
                    h4->SetBinContent(k,left[k]-left_background[k]);
                for (int k=150;k<165;k++)
                    h4->SetBinContent(k,0);
                for (int k=165;k<500;k++)
                    h4->SetBinContent(k,right[k-165]-right_background[k-165]);
                delete s1;
                delete s2;
                TSpectrum* s1 = new TSpectrum();
                TCanvas* c1 = new TCanvas();
                Int_t npeaks=s1->Search(h4,2.5,"goff",0.7);
                h1->SetLineColor(4);
                //h1->GetYAxis->SetRange(-60,15);
                h1->Draw();
                h2->SetLineColor(3);
                h2->Draw("same");
                h3->SetLineColor(2);
                ...
                TH1F* h5 = new TH1F("h5","peak_positions",500,0,500);
                for (int k=0;k<500;k++)
                    h5->SetBinContent(k,-100);
